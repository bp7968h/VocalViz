use crate::whisper::{SOT_TOKEN, model::Whisper as Model};
...
pub fn detect_language(model: &mut Model, tokenizer: &Tokenizer, mel: &Tensor) -> Result<u32> {
    ...
    let language_token_ids = LANGUAGES
        .iter()
        .map(|(t, _)| utils::token_id(tokenizer, &format!("<|{t}|>")))
    ...
    println!("🔍 Top Five Detected Language:");
    for (idx, ((_, language), p)) in probs.iter().take(5).enumerate() {
        if idx == 0 {
            println!("{:>5} {}: {}", "⭐", language, p);
        } else {
            println!("{:>5} {language}: {p}", idx + 1);
        }
    }
    let language = utils::token_id(tokenizer, &format!("<|{}|>", probs[0].0.0))?;
